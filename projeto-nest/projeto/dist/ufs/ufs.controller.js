"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UfsController = void 0;
const common_1 = require("@nestjs/common");
const ufs_service_1 = require("./ufs.service");
const create_uf_dto_1 = require("./dto/create-uf.dto");
const update_uf_dto_1 = require("./dto/update-uf.dto");
let UfsController = class UfsController {
    ufsService;
    constructor(ufsService) {
        this.ufsService = ufsService;
    }
    create(createUfDto) {
        return this.ufsService.create(createUfDto);
    }
    findAll() {
        return this.ufsService.findAll();
    }
    async findOne(id) {
        const uf = await this.ufsService.findOne(id);
        if (!uf)
            throw new common_1.NotFoundException();
        return uf;
    }
    async update(id, updateUfDto) {
        const uf = await this.ufsService.update(id, updateUfDto);
        if (!uf)
            throw new common_1.NotFoundException();
        return uf;
    }
    async remove(id) {
        const uf = await this.ufsService.remove(id);
        if (!uf)
            throw new common_1.NotFoundException();
    }
};
exports.UfsController = UfsController;
__decorate([
    (0, common_1.Post)(),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [create_uf_dto_1.CreateUfDto]),
    __metadata("design:returntype", void 0)
], UfsController.prototype, "create", null);
__decorate([
    (0, common_1.Get)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], UfsController.prototype, "findAll", null);
__decorate([
    (0, common_1.Get)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], UfsController.prototype, "findOne", null);
__decorate([
    (0, common_1.Patch)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, update_uf_dto_1.UpdateUfDto]),
    __metadata("design:returntype", Promise)
], UfsController.prototype, "update", null);
__decorate([
    (0, common_1.Delete)(':id'),
    (0, common_1.HttpCode)(204),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], UfsController.prototype, "remove", null);
exports.UfsController = UfsController = __decorate([
    (0, common_1.Controller)('ufs'),
    __metadata("design:paramtypes", [ufs_service_1.UfsService])
], UfsController);
//# sourceMappingURL=ufs.controller.js.map